...
> **Внимание:** файл `.env` содержит ключи доступа и должен оставаться исключённым из системы контроля версий (он уже добавлен в `.gitignore`). Не публикуйте его в удалённый репозиторий.

---

## Дорожная карта

- [x] EDA, feature engineering и обучение моделей (Logistic Regression, LightGBM).
- [x] Экспорт базовой модели в ONNX и логирование в MLflow.
- [x] REST API для прогноза с чтением данных из S3.
- [ ] Реализовать генерацию синтетических признаков (`src/utils/generate_synth_feature.py`).
- [ ] Автоматизировать переобучение модели и регистрацию run в MLflow.
- [ ] Добавить unit-тесты для `DataPreparation`, `PredictModel`, `DataManager`.
- [ ] Добавить обработку ошибок и логирование в едином формате.
